...
class BaseUrl
{
    ...
    {
        $route = cui::getAlias((string) $route);
        ...
        // relative route
        if (cui::$app->controller === null) {
            throw new InvalidParamException("Unable to resolve the relative route: $route. No active controller is available.");
        }

        if (strpos($route, '/') === false) {
            // empty or an action ID
            return $route === '' ? cui::$app->controller->getRoute() : cui::$app->controller->getUniqueId() . '/' . $route;
        } else {
            // relative to module
            return ltrim(cui::$app->controller->module->getUniqueId() . '/' . $route, '/');
        }
    }
    ...
    public static function to($url = '', $scheme = false)
    {
        ...
        $url = cui::getAlias($url);
        if ($url === '') {
            $url = cui::$app->getRequest()->getUrl();
        }
    ...
    public static function remember($url = '', $name = null)
    {
        ...
        if ($name === null) {
            cui::$app->getUser()->setReturnUrl($url);
        } else {
            cui::$app->getSession()->set($name, $url);
        }
    }
    ...
    {
        if ($name === null) {
            return cui::$app->getUser()->getReturnUrl();
        } else {
            return cui::$app->getSession()->get($name);
        }
    }
    ...
    {
        $params = cui::$app->controller->actionParams;
        $params[0] = cui::$app->controller->getRoute();
    ...
    {
        $currentParams = cui::$app->getRequest()->getQueryParams();
        $currentParams[0] = '/' . cui::$app->controller->getRoute();
    ...
    {
        return static::$urlManager ?: cui::$app->getUrlManager();
    }
}
